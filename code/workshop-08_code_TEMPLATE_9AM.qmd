...
drought_exp <- read_xlsx(
  # write in file path to data excel file
...
drought_exp_clean <- drought_exp |> # starting with the drought_exp object
  ...
  clean_names() |> 
  # adding new column for scientific species name
  ...
  # creating a new column with full names for water treatment
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |> 
  # making sure that species_name is a factor and reordering the levels
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As SLA increases, total plant mass tends to increase (positive relationship)")
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "Mean plant mass is higher in \n well watered than in \n drought stressed treatment") +
  # taking out legend
...
AICc(
  ...
  model2, 
  model3, 
  model4, # water treatment and species lowest
  model5, 
  model6, 
  model7) |> #arranging output in descending order of AIC
  arrange(AICc)
```
